...
// 生产边界：正式AIBrainBox部署优先由上海交大ROS 2节点调用C ABI，本协议只用于本地UDP演示、
// GCS联调和事件日志；所有多字节字段按小端编码，浮点按IEEE-754位模式传输。
//
// C++初学者阅读提示：
// 1. 网络只能发送字节；“编码”把有名字的字段变成固定顺序的字节，“解码”执行相反过程。
// 2. FrameHeader是程序内便于使用的对象，不应直接用memcpy发送，因为编译器可能插入填充字节。
// 3. MessageType说明payload是哪一种业务内容；decode前必须先检查类型、长度、版本和CRC。
// 4. 小端规定多字节整数的最低有效字节先发送，使Windows与RK3588得到同一线序结果。
// 5. `std::variant`表示一个载荷对象在同一时刻只能是列出的某一种类型；
//    `std::get<T>`按具体类型取值，类型与MessageType不一致会被编码入口拒绝。
#pragma once
