// 模块职责：给上海交大集成人员演示不依赖ROS 2类型的C ABI最小生命周期。
// 示例刻意只展示结构初始化、会话创建、测距输入、两阶段step和销毁；正式默认IMU+测距
// 还须在首个输入前调用zju_coop_configure_inertial并持续输入各节点瞬时IMU。
//
// C++初学者阅读提示：
// 1. 本文件是“怎样调用库”的示例，不是算法核心；建议从main顶部按阶段1到阶段5顺序阅读。
// 2. std::array长度固定，std::vector长度可在运行时确定；data()取得传给C函数的首元素指针。
// 3. 所有zju_coop_*_init都在填写业务字段前调用；任何错误都先打印并停止，避免使用无效句柄。
// 4. 示例为突出接口生命周期只输入测距，实机默认路径必须同时配置并连续输入各车IMU。
// 5. `static_cast`用于显式整数转换；`constexpr`表示数组在编译期已确定；
//    `auto`让编译器从函数返回值推导准确类型，但变量物理含义仍由旁边注释说明。
